...
async fn tcp_server_loop(
    ...
    loop {
        ...
        tauri::async_runtime::spawn(async move {
            if let Err(e) =
                handle_new_connection(stream, src_addr, app_handle_clone, state_clone).await
            {
...
async fn register_connection(
    ...
    (Uuid, Arc<Mutex<tokio::net::tcp::OwnedWriteHalf>>),
    ...
    if let Some(existing) = pool.get(&peer_id) {
        if existing.is_outbound != is_outbound {
            ...
            if we_are_connector {
                if !is_outbound {
                    return Err(
                        "Declining inbound connection due to outbound preference tie-breaker"
                            .into(),
                    );
                }
            } else {
                if is_outbound {
                    return Err(
                        "Declining outbound connection due to inbound preference tie-breaker"
                            .into(),
                    );
                }
            }
...
async fn handle_new_connection(
    ...
    if envelope.msg_type == "file_request" {
        ...
        let (connection_id, _write_half_arc) =
            match register_connection(state.clone(), peer_id, write_half, false).await {
                Ok(res) => res,
                Err(e) => {
                    println!(
                        "Inbound connection from {} rejected by tie-breaker: {}",
                        peer_id, e
                    );
                    return Ok(());
                }
...
pub async fn send_chat_message(
    ...
    if let Some(write_half_arc) = existing_conn {
        ...
        if framing::write_frame(&mut *write_half, &frame_bytes)
            .await
            .is_ok()
        {
            return Ok(());
    ...
    let (connection_id, write_half_arc) =
        register_connection(state.clone(), peer_id, write_half, true).await?;
